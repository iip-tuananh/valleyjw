...
<head>
    ...
    {{-- <script>
        window.file_url = "//file.hstatic.net/200000541929/file/";
        window.asset_url = "//theme.hstatic.net/200000541929/1001190790/14/?v=1803";
...
<body id="hogwarts-theme" class="index" ng-app="App" ng-cloak>
    <div class="nav-overlay" onclick="initNav('close')"></div>
    <div class="cart-overlay" onclick="initCart('close')"></div>
    <div class="login-overlay" onclick="initLogin('close')"></div>
    <div class="menu-mobile-overlay" onclick="initNav('close')"></div>
    ...
    <div class="dlpm-hrvmodal dlpm-hrvmodal--dialog dlpm-hrvmodal-coupon" ng-controller="PopupVoucherController">
        <div class="dlpm-hrvmodal--content">
            <div class="dlpm-hrvmodal--header">
                <div class="dlpm-hrvmodal-base--list">
                    <div class="dlpm-hrvmodal--header__title">Chi tiết mã giảm</div>
                    <div class="dlpm-hrvmodal-close btn-hrvmodal-coupon__close">
            ...
            <hr>
            <div class="dlpm-hrvmodal--body">
                <div class="dlpm-detail-items">
                    <div class="detail-info">
                        <div class="detail-info__label"><% item_coupon.name %></div>
                        <div class="detail-description__1"><% item_coupon.description %></div>
                    </div>
                    <hr>
                    <div class="detail-code">
                        <div class="detail-code__label">Mã giảm giá</div>
                        <div class="detail-code__copy">
                            <div class="detail-code__copy-label"><% item_coupon.code %></div>
                            <div class="detail-code__copy-btn" ng-click="copyCouponCode(item_coupon.code)">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M13.75 0.833344H3.74998C2.83331 0.833344 2.08331 1.58334 2.08331 2.50001V14.1667H3.74998V2.50001H13.75V0.833344ZM16.25 4.16668H7.08331C6.16665 4.16668 5.41665 4.91668 5.41665 5.83334V17.5C5.41665 18.4167 6.16665 19.1667 7.08331 19.1667H16.25C17.1666 19.1667 17.9166 18.4167 17.9166 17.5V5.83334C17.9166 4.91668 17.1666 4.16668 16.25 4.16668ZM16.25 17.5H7.08331V5.83334H16.25V17.5Z"
                                        fill="#2463EB"></path>
                                </svg>
                            </div>
                        </div>
                    ...
                    <div class="detail-datetime">
                        <div class="detail-datetime__label">Áp dụng từ</div>
                        <div class="detail-datetime__content"><% item_coupon.from_date | date:'dd/MM/yyyy' %> - <% item_coupon.to_date | date:'dd/MM/yyyy' %></div>
                    </div>
                    <hr>
                    <div class="detail-content">
                        <div class="detail-content__label">Chi tiết</div>
                        <div class="detail-content__description" ng-bind-html="item_coupon.content"></div>
                    </div>
                </div>
            </div>
            <div class="dlpm-hrvmodal--footer">
                <hr>
                <div class="dlpm-hrvmodal--footer__action">
                    <div class="dlpm-footer--action__close btn-hrvmodal-coupon__close">Đóng</div>
                    <div class="dlpm-footer--action__copy dlpm-btn-copy" ng-click="copyCouponCode(item_coupon.code)">Sao chép</div>
                </div>
            </div>
        </div>
    </div>
    <script src='/site/js/plugins.js?v=1803' type='text/javascript'></script>
    <script src='/site/js/scripts.js?v=1803' type='text/javascript'></script>
    @stack('script')
    <script>
        setTimeout(function () {
            animation_check();
        ...
        function copyCouponCode(code) {
            ...
            toastr.success('Sao chép mã thành công !');
        }
    ...
    <script>
        $(document).ready(function () {
            ...
            $(window).scroll(function () {
                var headerOffset = $('.header-main').next().offset().top;
    ...
    <script>
        app.controller('PopupVoucherController', function ($scope, $http) {
            $scope.item_coupon = {};
            $('.btn-hrvmodal-coupon').on('click', function () {
                $scope.item_coupon = $(this).data('item');
                $scope.$applyAsync();
                $('.dlpm-hrvmodal-coupon').addClass('open');
                $('.dlpm-hrvmodal-coupon').before('<div class="dlpm-hrvmodal--backdrop phzttukuap"></div>');
            });

            $('.btn-hrvmodal-coupon__close').on('click', function () {
                $('.dlpm-hrvmodal-coupon').removeClass('open');
                $('.dlpm-hrvmodal--backdrop').remove();
            });

            $scope.copyCouponCode = function (code) {
                navigator.clipboard.writeText(code);
                toastr.success('Sao chép mã thành công !');
            }
        });
    ...
    <script>
        app.directive('homeFeaturedProduct', function($compile) {
            return {
                link: function(scope, element) {
                    $('.home-featured-product').each(function () {
                        let _self = $(this);
                        let _wrap = _self.find('.loadmore-content');
                        let handle = _self.data('handle');
                        if (handle != '') {
                            $.ajax({
                                url: '/load-product-home-page?view=ajax-index&page=1&handle=' + handle,
                                success: function (data) {
                                    _wrap.empty();
                                    _wrap.append(data.html);
                                    $compile(_wrap.contents())(scope); // Biên dịch lại các phần tử được chèn để Angular nhận diện
                                }
                            });
                        }
                    });
        ...
        app.controller('ListProductController', function ($rootScope, $scope, cartItemSync, $interval) {
            ...
            // jQuery('#live-search-mobile').keyup(function() {
            //     var keyword = jQuery(this).val();
            //     jQuery.ajax({
            //         type: 'post',
            //         url: '{{route("front.auto-search-complete")}}',
    ...
    <script>
        ...
        var quickViewProduct = function (purl) {
            ...
            $.ajax({
                url: '/get-product-quick-view?handle=' + purl,
                async: false,
                success: function (product) {
                    modal.find('#form-quick-view').html(product.html);
                }
            });
    ...
    <script>
        $(document).ready(function () {
            var orders = ["clara", "dien-truc", "ella", "ostara", "rosa",];


            var sale_customers = ["Thảo My ở Hồ Chí Minh", "Thu Thảo ở Hà Nội", " Ngân Nguyễn ở Hà Tây", "Đỗ Tùng ở Hà Nội", "Thu Thuỷ ở Hồ Chí Minh", " Thanh Bình ở Hồ Chí Minh", "Hà Thu ở Hồ Chí Minh", "Ngo Dung ở Vũng Tàu", " Thảo Điền ở Hải Dương", "Thu Đỗ ở Đà Nẵng", "Mơ Thị Nguyễn ở Long An", " Thu Trang ở Hà Nội", "Cẩm Vân ở Hà Nội", "Hoàng Long ở Thanh Hoá", " Dương Nguyễn ở Nha Trang", "Mai Trang ở Sơn La", "Ngọc Lan ở Huế", " Mộc Miên ở Bình Dương", "Thuy Do ở Quảng Bình", "Bích Liên ở Nghệ An", " Thu Quỳnh ở Hà Nội", "Hoa Nguyen ở Hà Nam", "Cẩm Nhung ở Bình Dương", " Thu Hương ở Hồ Chí Minh", "Hà Ngân ở Cà Mau", "Ngọc Nguyễn ở Vinh", "Hà Linh ở Lâm Đồng", "Nguyễn Thị Lam ở Bắc Ninh", "Ngọc Linh ở Bình Dương", "Ngọc Nguyễn ở Bình Định", "Thu Hạnh ở Sơn Tây"];
            var sale_time_ago = ["Cách đây 30 phút", "5 phút trước", "7 phút trước", "Cách đây 20 phút", "5 phút trước", "vừa đặt hàng", "Cách đây 10 phút", "2 phút trước", "15 phút trước", "Cách đây 1 phút", "2 phút trước", "Cách đây 8 phút"];

            var count_show = 20;
            var count_increase = 1;

            var time_delay_between = 15000;
            var time_delay_popup = 5000;


            var interval = setInterval(function () {
                var item = orders[Math.floor(Math.random() * orders.length)];
                ...
                $.get("/laster-buy-products").done(function (data) {
                    var sale_template = `
                        ...
                        ${customer} đã đặt hàng <a href="/san-pham/${data.product.slug}.html"><b>${data.product.name}</b></a>
                        <small>${time}</small>
                        </p>
                        </div>
                        </div>
            ...
            $("body").on('click', '.custom-close', function () {
                $(this).parents(".custom-social-proof").stop().slideToggle('slow');
            })
        })
    </script>
    <style>
    ...
    <!-- <script>
        ...
        chatbox.setAttribute("page_id", "102680835648441");
    ...
    {{-- <script>window.OniReviewConfig = { "shop": { "active": true, "insider_id": 237, "myharavan_domain": "https://theyen.myharavan.com", "cdn_token": "clw77y6gk10960so3y35c5jke" }, "config": { "reviewTitle": "Danh sách đánh giá", "questionTitle": "Câu hỏi thường gặp", "showTitle": true, "showDate": false, "showSort": true, "acceptImage": true, "acceptVideo": true, "acceptQuestion": true, "questionType": "tab", "acceptReply": true, "acceptVerifiedBadge": false, "alwaysVerifiedBadge": false, "shapeType": "star_1", "colorShape": "#ffb800", "colorBgShape": "#b3bcc5", "colorVerifiedBadge": "#01ab56", "colorMain": "#BB0909", "requireLogin": false } }</script> --}}
    {{-- <script defer src="https://assets.onistudio.net/static/onireviews.js"></script> --}}
    <!-- <script src="https://chat-plugin.pancake.vn/main/auto?page_id=web_web"></script> -->
</body>
